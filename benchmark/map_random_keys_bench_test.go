...
// Comparison targets: cc.Map, cc.FlatMap, sync.Map, xsync.MapV4, skipmap, haxmap, RWLockShardedMap
// ============================================================================
...
// ============================================================================
// Mixed operation tests (read-write mix, closer to real-world scenarios)
// Ratio: Store 12.5%, Delete 12.5%, Load 75%
// ============================================================================

func BenchmarkConcurrent_IntMixed_ccMap(b *testing.B) {
	m := cc.NewMap[int, int]()
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_ccFlatMap(b *testing.B) {
	m := cc.NewFlatMap[int, int]()
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_syncMap(b *testing.B) {
	var m sync.Map
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_skipmap(b *testing.B) {
	m := skipmap.New[int, int]()
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_haxmap(b *testing.B) {
	m := haxmap.New[int, int]()
	for _, key := range preGenIntKeys {
		m.Set(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_xsyncMap(b *testing.B) {
	m := xsync.NewMap[int, int]()
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_IntMixed_RWShardedMap(b *testing.B) {
	m := NewRWLockShardedMap[int, int](runtime.GOMAXPROCS(0) * 4)
	for _, key := range preGenIntKeys {
		m.Store(key, key)
	}
...
func BenchmarkConcurrent_StrMixed_ccMap(b *testing.B) {
	m := cc.NewMap[string, int]()
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_ccFlatMap(b *testing.B) {
	m := cc.NewFlatMap[string, int]()
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_syncMap(b *testing.B) {
	var m sync.Map
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_skipmap(b *testing.B) {
	m := skipmap.NewString[int]()
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_haxmap(b *testing.B) {
	m := haxmap.New[string, int]()
	for i, key := range preGenStringKeys {
		m.Set(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_xsyncMap(b *testing.B) {
	m := xsync.NewMap[string, int]()
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
...
func BenchmarkConcurrent_StrMixed_RWShardedMap(b *testing.B) {
	m := NewRWLockShardedMap[string, int](runtime.GOMAXPROCS(0) * 4)
	for i, key := range preGenStringKeys {
		m.Store(key, i)
	}
