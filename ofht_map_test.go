...
func TestOFHTMapLongProbeLimitSurvivesResize(t *testing.T) {
	...
	const n = ofhtMaxProbeThreshold + 1
	for i := range n {
		m.Store(i, i*10)
	}
	for i := range n {
...
func TestOFHTMapLongProbeLimitRepeatedResizeBadHash(t *testing.T) {
	m := NewOFHTMap[int, int](
		WithCapacity(1),
		WithKeyHasher(func(int, uintptr) uintptr { return 0 }),
	)
	...
	const n = ofhtMaxProbeThreshold * 16
	for i := range n {
		m.Store(i, i+1)
	}
	for i := range n {
	...
	if table.probeLimit < uintptr(ofhtMaxProbeThreshold) {
		t.Fatalf("probeLimit=%d, want >= %d", table.probeLimit, ofhtMaxProbeThreshold)
	}
}

func TestOFHTMapLongProbeLimitConcurrentBadHashMixed(t *testing.T) {
	m := NewOFHTMap[int, int](
		WithCapacity(1),
		WithKeyHasher(func(int, uintptr) uintptr { return 0 }),
	)
